...
def generate_btc_data(n_days: int = 1460, start_date: str = '2021-01-01') -> pd.DataFrame:
    ...
    for t in range(1, n_days):
        ...
        # 3% chance of a jump
        jump = np.random.choice([0, 0.05, -0.08], p=[0.97, 0.02, 0.01])
    ...
    # --- 2. Volume ---
    df['volume'] = np.random.lognormal(mean=np.log(
        2.5e10), sigma=0.5, size=n_days)  # High volume base

    # --- 3. Technical Features ---
    df['volatility_30d'] = df['log_return'].rolling(
        window=30).std() * np.sqrt(365)  # Annualized
    ...
    df['rsi_14'] = 50 + 20 * (df['log_return'].rolling(window=14).sum() / df['log_return'].rolling(
        window=14).std().replace([np.inf, -np.inf], np.nan).fillna(0.1))
    ...
    df['active_addresses'] = np.random.lognormal(
        mean=np.log(5e5), sigma=0.3, size=n_days)
    # Hash rate: proxy for network security, tends to increase over time
    df['hash_rate'] = np.random.lognormal(mean=np.log(
        2e8), sigma=0.1, size=n_days) * (1 + 0.001 * np.arange(n_days))
    # Exchange inflow: selling pressure indicator
    df['exchange_inflow'] = np.random.lognormal(
        mean=np.log(1e4), sigma=0.8, size=n_days)
    # Whale transactions: large transactions, Poisson distributed for discrete events
    df['whale_transactions'] = poisson.rvs(
        mu=15, size=n_days)  # Avg 15 whale tx per day

    # --- 5. Sentiment (from social media NLP) ---
    # Twitter sentiment: influenced by recent returns
    df['twitter_sentiment'] = np.clip(np.random.randn(
        n_days)*0.3 + 0.1 * df['log_return'].shift(1) * 20, -1, 1)
    ...
    df['fear_greed_index'] = np.clip(
        50 + 30*np.random.randn(n_days) + 200*df['log_return'].shift(1), 0, 100)
...
def preprocess_data(
    ...
    X_seq_train, y_seq_train = prepare_sequences(
        X_scaled[:split_index], y[:split_index], sequence_length)
    X_seq_test, y_seq_test = prepare_sequences(
        X_scaled[split_index:], y[split_index:], sequence_length)
    ...
    test_log_returns_lstm = btc_data['next_day_return'].iloc[split_index +
                                                             sequence_length:].values

    print(
        f"Data split: {split_index} days for training, {len(btc_data) - split_index} days for testing.")
    print(
        f"Random Forest training features shape: {X_train_rf.shape}, target shape: {y_train_rf.shape}")
    print(
        f"Random Forest testing features shape: {X_test_rf.shape}, target shape: {y_test_rf.shape}")
    print(
        f"LSTM training sequences shape: {X_seq_train.shape}, target shape: {y_seq_train.shape}")
    print(
        f"LSTM testing sequences shape: {X_seq_test.shape}, target shape: {y_seq_test.shape}")
...
def build_lstm_model(n_features: int, sequence_length: int = SEQUENCE_LENGTH) -> tf.keras.Model:
    ...
    """
    model = Sequential([
        LSTM(units=64, input_shape=(sequence_length, n_features),
...
def evaluate_crypto_model(
    ...
    metrics['sharpe_ratio'] = (
        daily_avg_return / max(daily_std_dev, 1e-6)) * np.sqrt(365)
    ...
    metrics['bh_sharpe_ratio'] = (
        bh_daily_avg_return / max(bh_daily_std_dev, 1e-6)) * np.sqrt(365)

    # Store strategy and buy-and-hold cumulative returns for equity curve
    metrics['strategy_cumulative_returns'] = np.cumprod(
        1 + strategy_simple_returns) - 1
    metrics['bh_cumulative_returns'] = np.cumprod(1 + bh_simple_returns) - 1

    print(f"\n--- {label} EVALUATION ---")
    print("=" * 50)
    print(
        f"Directional accuracy: {metrics['directional_accuracy']:.2%} (random = 50%)")
    ...
    print(
        f"AI edge over B&H (Sharpe): {metrics['sharpe_ratio'] - metrics['bh_sharpe_ratio']:.2f}")
...
def train_and_evaluate_models(
    ...
    lstm_model = build_lstm_model(
        n_features=n_features_lstm, sequence_length=sequence_length)
    ...
    rf_probs = rf_model.predict_proba(
        X_test_rf)[:, 1]  # Probability of class 1 (up)
    # Convert probabilities to binary predictions
    rf_preds = (rf_probs > 0.5).astype(int)

    # Evaluate models
    lstm_eval_metrics = evaluate_crypto_model(
        y_seq_test, lstm_preds, lstm_probs, test_log_returns_lstm, 'LSTM')
    rf_eval_metrics = evaluate_crypto_model(
        y_test_rf, rf_preds, rf_probs, test_log_returns_rf, 'Random Forest')
...
def analyze_feature_importance(
    ...
    importance_df = pd.Series(rf_model.feature_importances_, index=all_features).sort_values(
        ascending=False).to_frame(name='importance')

    # Add feature group for visualization
    importance_df['group'] = importance_df.index.map(lambda x: next(
        (group_name for group_name, features in feature_groups.items() if x in features), 'Other'))
    ...
    group_importance = importance_df.groupby(
        'group')['importance'].sum().sort_values(ascending=False)
    ...
    sns.barplot(x='importance', y=importance_df.index, hue='group',
                data=importance_df, dodge=False, palette='viridis', ax=ax)
...
def visualize_results(
    ...
    sns.histplot(lstm_probs, bins=20, kde=True, color='skyblue',
                 label='LSTM Probabilities', ax=ax_confidence)
    sns.histplot(rf_probs, bins=20, kde=True, color='lightcoral',
                 label='Random Forest Probabilities', alpha=0.7, ax=ax_confidence)
    ...
    ax_confidence.axvline(0.5, color='gray', linestyle='--',
    ...
    ax_equity.plot(btc_data['date'].iloc[split_index + sequence_length:],
                   lstm_eval_metrics['strategy_cumulative_returns'], label='LSTM Strategy', color='blue')
    # Random Forest strategy and Buy-and-Hold dates start after split_index
    ax_equity.plot(btc_data['date'].iloc[split_index:],
                   rf_eval_metrics['strategy_cumulative_returns'], label='Random Forest Strategy', color='green')
    ax_equity.plot(btc_data['date'].iloc[split_index:], rf_eval_metrics['bh_cumulative_returns'],
                   label='Buy-and-Hold Baseline', color='red', linestyle='--')
...
def honest_assessment() -> str:
    ...
    assessment_lines.append(
        " - Apply CFA Standard V(A) - Diligence and Reasonable Basis: Always exercise due diligence on AI-driven claims as with any investment product.")
    assessment_lines.append("=" * 70)
...
def run_crypto_prediction_pipeline(
    ...
    print(
        f"Base rate (% up days): {btc_data['next_day_direction'].mean():.2%}")
    ...
    importance_df, feature_importance_fig = analyze_feature_importance(
        rf_model, ALL_FEATURES, FEATURE_GROUPS)
